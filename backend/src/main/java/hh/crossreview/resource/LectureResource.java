...
public class LectureResource {
    ...
    public Response createLecture(LecturePostDto lecturePostDto, @Context SecurityContext securityContext) {
        ...
        var lecture = lectureService.createLecture(lecturePostDto, jwtTokenUtils.retrieveTokenFromContext(securityContext), lector);
    ...
    @ApiResponse(
            ...
            description = "Bad request",
            content = @Content(schema = @Schema(implementation = ExceptionValidationDto.class)))
    public Response patchLecture(@PathParam("lectureId") Integer lectureId, LecturePatchDto lecturePatchtDto, @Context SecurityContext securityContext) {
        var lector = userService.findByPrincipal(securityContext.getUserPrincipal());
